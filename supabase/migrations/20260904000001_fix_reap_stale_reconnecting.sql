-- MIGRATION: 20260904000001_fix_reap_stale_reconnecting.sql
-- DESCRIPTION: State-specific stale stream reaper with worker lease detection and node cleanup
  ...
  FOR v_stream IN
    ...
    WHERE s.status = 'stopping'
      ...
      AND (
        ...
        OR (
          NOT EXISTS (
            SELECT 1 FROM public.stream_telemetry st 
            WHERE st.stream_id = s.id 
              AND st.recorded_at > now() - interval '2 minutes'
          )
