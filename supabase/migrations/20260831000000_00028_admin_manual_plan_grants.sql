-- =========================================================================================
-- 00028: Admin Manual Plan Grants (Agency Plan Assignment Without Payment)
-- =========================================================================================

-- 1. Create billing_plan_grants Table
CREATE TABLE IF NOT EXISTS public.billing_plan_grants (
...
-- Index for rapid active grant lookup
CREATE INDEX IF NOT EXISTS idx_billing_plan_grants_active 
    ON public.billing_plan_grants (user_id, starts_at, expires_at)
    WHERE revoked_at IS NULL;

-- 2. Row Level Security on billing_plan_grants
ALTER TABLE public.billing_plan_grants ENABLE ROW LEVEL SECURITY;

-- Users can view their own grants; Admins can view all
CREATE POLICY "Users can view own grants" 
    ON public.billing_plan_grants FOR SELECT 
    TO authenticated 
    USING (auth.uid() = user_id OR public.is_admin());

-- Only admins can directly insert or update
CREATE POLICY "Admins can insert grants" 
    ON public.billing_plan_grants FOR INSERT 
    TO authenticated 
    WITH CHECK (public.is_admin());

CREATE POLICY "Admins can update grants" 
    ON public.billing_plan_grants FOR UPDATE 
    TO authenticated 
    USING (public.is_admin())
    WITH CHECK (public.is_admin());


-- 3. Authoritative get_effective_entitlements Function with Deterministic Precedence
-- Precedence: 1. Active Admin Manual Grant -> 2. Active Stripe Subscription -> 3. Free Fallback
...
DECLARE
    ...
    v_sub RECORD;
BEGIN
    -- 1. Check for an active admin manual grant
    SELECT 
        bpg.id AS grant_id,
        bpg.plan_id,
        bpg.expires_at,
    ...
    WHERE bpg.user_id = p_user_id
      AND bpg.revoked_at IS NULL
      AND bpg.starts_at <= now()
      AND (bpg.expires_at IS NULL OR bpg.expires_at > now())
    ...
    IF FOUND THEN
        ...
        SELECT 
            ...
            'admin_grant'::TEXT AS entitlement_source,
            v_grant.grant_id,
            v_grant.expires_at AS grant_expires_at,
            v_grant.reason AS grant_reason
        FROM public.billing_plans bp
        WHERE bp.id = v_grant.plan_id;
        RETURN;
    END IF;

    -- 2. Check for an active, trialing, or past_due Stripe subscription
    SELECT 
        s.plan_id, 
        s.status, 
    ...
    IF FOUND THEN
        ...
        SELECT 
            ...
            bp.max_fps,
            bp.advanced_analytics,
            'stripe'::TEXT AS entitlement_source,
    ...
    -- 3. Implicit fallback to 'free' plan
    RETURN QUERY
    SELECT 
        bp.id AS plan_id,
        bp.name AS plan_name,
        ...
        bp.max_fps,
        bp.advanced_analytics,
        'free'::TEXT AS entitlement_source,
        NULL::UUID AS grant_id,
        NULL::TIMESTAMPTZ AS grant_expires_at,
        NULL::TEXT AS grant_reason
    FROM public.billing_plans bp
...
-- 5. Admin RPC: admin_revoke_plan_grant
...
-- 6. Admin RPC: admin_list_user_plan_grants
